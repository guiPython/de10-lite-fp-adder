...
architecture arch of fp_adder is
    -- Suffixes b, s, a and n mean big, small, aligned and normalized.
    signal signb, signs             : std_logic;
    signal expb, exps, expn         : unsigned(3 downto 0);
    signal fracb, fracs, fraca      : unsigned(7 downto 0);
    signal fracn, sum_norm          : unsigned(7 downto 0);
...
begin
    ...
    -- 2nd stage: align the smaller number.
    exp_diff <= expb - exps;
    ...
    -- 3rd stage: add or subtract the aligned significands.
    sum <= ('0' & fracb) + ('0' & fraca) when signb = signs else
           ('0' & fracb) - ('0' & fraca);

    -- 4th stage: count leading zeros.
    ...
    -- Shift the significand left according to the leading-zero count.
    with leado select
        sum_norm <=
    ...
    -- Normalize and handle carry-out and underflow conditions.
